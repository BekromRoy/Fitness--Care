...
if(isset($_POST['res_submit'])){
          ...
          if (isset($_GET['token'])) {
             ...
             $newpassword =  mysqli_real_escape_string($con,$_POST['res_pass']);
             $cpassword =  mysqli_real_escape_string($con,$_POST['res_cpass']);
           ...
           if($newpassword === $cpassword){
                    ...
                    $updatequery = " update fitness_table set password='$pass' where token='$re_token' ";
                    $iquery = mysqli_query($con,$updatequery);
          
                    if($iquery){

                              ?> <script>alert("Password has been updated succssfully. Click OK to continue.");</script> <?php

                              header('location:../dashboard/dashboard.php');
                    }else {
                              $_SESSION['rmsg'] = "Password is failed to updated.";
                
                        }
                    }else{
                              $_SESSION['rmsg'] = "Passwords are not same.";
           }
    }else {
      $_SESSION['rmsg'] = "Token not found.";
    }
}
...
<head>
  ...
  <link rel="icon" type="image/png" href="../logo/logo.png">
          <title>Reset | Fitness Care</title>
          <style>
                    body{
                              display: flex;
...
.active {
  border-bottom: 1 px solid gray;
    ...
    .circle1 {
      ...
      width: 600px;
      height: 600px;
      background: -webkit-linear-gradient(to bottom, #ffc0cb, #800080);
      background: linear-gradient(to bottom, #ffc0cb, #800080);
      z-index: -1;
      border-radius: 50%;
      top: 0%;
      left: 0%;
...
<!--form part-->
   <div class="container-fluid" style="height: ;position: relative;padding-top: 80px; padding-bottom: 80px;">
     ...
     <div class="row px-3">
        ...
        <div class="card-body"> 
          <h2 class="title text-center mt-4" style="color:#007bff;">Reset your password here.</h2>
          <div>
      ...
      <form class="form-box px-3" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post">
        ...
        <div class="form-input">
          <span><i class="fa fa-key"></i></span>
            <input type="password" name="res_pass"  id="password" placeholder="Creare password more than 6 characters" required>
            <div id="showhide" onclick="showHide();"></div>
                    </div>
                    <!-- For comfirm password input -->
        <div class="form-input">
          <span><i class="fa fa-key"></i></span>
            <input type="password" name="res_cpass"  id="cpassword" placeholder="Enter comfirm password" required>
            <div id="cshowhide" onclick="showHide2();"></div>
                    </div>
                    <!-- For submit button -->
        <div class="mb-3">
            <button type="submit" name="res_submit" class="btn btn-primary text-center btn1"> Reset Password</button>
                    </div>
...
<script src="../denie/denie.js"></script>
      ...
      <script>
         ...
         $("input").focus(function(){
          ...
          const cshowhide = document.getElementById('cshowhide');
          function showHide(){
          ...
          function showHide2(){
            if (cpassword.type === 'password') {
              ...
              cshowhide.classList.add('hide')
            }
            else {
              cpassword.setAttribute('type','password');
              cshowhide.classList.remove('hide')
            }
          }
