     ...
     if($emailcount){
              // SENDING EMAIL FOR ACCOUNT ACTIVATION
                  ...
                  $token = $userdata['token'];
                    ...
                    $subject = "Forgot Password - FITNESS CARE";
                    $body = "Hello $username ,This is email from FITNESS CARE for Reset Password. Click on link to reset your password - http://fitnesscare.ultimatefreehost.in/login/reset.php?token=$token";
              ...
              if(mail($email, $subject, $body, $headers)){
                  $_SESSION['msg'] = "Reset password link has been sent to your email $email ";
                    }else{
                            $_SESSION['fmsg'] = "Email sending failed..";
                  }
                }else {
                  $_SESSION['fmsg'] = "Email not found.";
                }
              }
            }
   ?>
...
<head>
  ...
  <link rel="icon" type="image/png" href="../logo/logo.png">
          <title>Forgot | Fitness Care</title>
          <style>
                    body{
                              display: flex;
...
.active {
  border-bottom: 1 px solid gray;
           ...
           .img-left{
                    width: 45%;
                    background:url('forgot.png') center;
...
<!--form part-->
   <div class="container-fluid" style="height: ;position: relative;padding-top: 80px; padding-bottom: 80px;">
     ...
     <div class="row px-3">
        ...
        <div class="card-body"> 
          <h3 class="title text-center mt-4" style="color:#007bff;">Send a mail to reset password.</h3>
      ...
      <form class="form-box px-3" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post" autocomplete="on">
        ...
        <div class="form-input">
          <span><i class="fa fa-envelope-o"></i></span>
            <input type="email" name="forgot_email" id="" placeholder="Enter email here"  required>
                    </div>
                    
                    <!-- For submit button -->
        <div class="mb-3">
            <button type="submit" name="forgot_submit" class="btn btn-primary text-center btn1" style="padding: 20px auto;"> Send Mail</button>
                    </div>
                    </form>
             </div>
          </div>
